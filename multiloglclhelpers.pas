...
{§< Unit implementing TLoggerLCLHelper, a class helper for TLogger - like a TLogger's hack from its ouside - to add a management of messages like
@code(SendBitmap();), @code(SendColor();), which use visual structures from the Graphics unit, a base unit for the LCL graphic oriented components.
}
...
type
  ...
  TLoggerLCLHelper = class helper for TLogger
  private
  public
...
procedure TLoggerLCLHelper.SendBitmap(Classes: TDebugClasses; const AText: String; ABitmap: TBitmap);
var
  AStream: TStream;
begin
...
procedure TLoggerLCLHelper.SendColor(Classes: TDebugClasses; const AText: String; AColor: TColor);
begin
  if Classes * ActiveClasses = [] then Exit;
